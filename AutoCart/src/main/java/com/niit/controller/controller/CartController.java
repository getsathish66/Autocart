...
public String mycart(Principal p, Model model){
	...
	model.addAttribute("mycartList",mycartDAO.listCartByStatus(mail, "N"));
	
	model.addAttribute("isUserClickedaddtocart", "true");
	
	if(mycartDAO.listCartByStatus(mail, "N").isEmpty()){
...
public String addtocart(@RequestParam("productId") String productId , Principal p, Model model){
	...
	User user=userDAO.get(p.getName());
		Mycart mycrt = mycartDAO.getByUserandProduct(p.getName(), productName);

		
		if (product.getProductQuantity() > 0) {

			if (mycartDAO.itemAlreadyExist(p.getName(), productName, true)) {
